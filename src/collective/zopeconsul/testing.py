...
if HAS_REDIS and HAS_MSGPACK:
    from collective.zopeconsul import redisqueue
...
class TaskQueueServerLayer(Layer):
    ...
    def setUp(self):
        ...
        xmlconfig.file('configure.zcml', collective.zopeconsul,
                       context=self['configurationContext'])

        # Configure
        config = getConfiguration()
        config.product_config = {'collective.zopeconsul': {'queue': self.queue}}
        taskqueue.reset()
        ...
        from collective.zopeconsul.server import TaskQueueServer
